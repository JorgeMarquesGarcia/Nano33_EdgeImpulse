...
/*Led definition */
#define LED_R 22  // Rojo
#define LED_G 23  // Verde
#define LED_B 24  // Azul
...
/*Control value to turn on led and definition of fuction to choose the led colour*/
float control_val = 0.2;
...
void loop()
{
    ...
    }/*Logic for led turn on 
            Positive roll -> result.classification[4].value -> Red
            Negative roll -> result.classification[3].value -> Pink
            Positive pitch -> result.classification[2].value -> Green
            Negative pitch -> result.classification[1].value -> Yellow
            Positive yaw -> result.classification[6].value -> Blue
            Negative yaw -> result.classification[5].value -> Purple
            resukt.classification[0].value -> No movement, labeled as NoiseUP
    
    */
        if (result.classification[4].value > control_val) {
        setColor(255, 0, 0);  // Red (Positive Roll)
          } else if (result.classification[3].value < control_val) {
        setColor(255, 0, 255);  // Pink (Negative Roll)
          } else if (result.classification[2].value > control_val) {
        setColor(0, 255, 0);  // Green (Positive Pitch)
          } else if (result.classification[1].value < control_val) {
        setColor(255, 255, 0);  // Yellow (Negative Pitch)
          } else if (result.classification[6].value > control_val) {
        setColor(0, 0, 255);  // Blue (Positive Yaw)
          } else if (result.classification[5].value < control_val) {
        setColor(128, 0, 128);  // Purple (Negative Yaw)
      } else {
        setColor(0, 0, 0);  
      }
...
/*In continuous measure, this makes a compìlation error. Found in edge-impulse forum how to change the sensor type, see file:///C:/Users/JooRg/OneDrive/Desktop/M%C3%A1ster%20electr%C3%B3nica/Sem%202/[01]%20Redes%20de%20sensores/[01]%20Pr%C3%A1cticas/Error%20while%20deploying%20accelorometer%20on%20arduino%20nano%2033%20ble%20-%20Help%20-%20Edge%20Impulse%20Forum.html
but this nos works also. */
